...
$(function () {
	...
	*/
	...
	$('.lnks').on('click', '.lnk.discover', function(e) {
		e.preventDefault();

		$('.top-menu a[href="#contacts-card"]').trigger('click', function (e) {
			e.preventDefault();
		});
	});
		...
		submitHandler: function() {
			$.ajax({
				...
				success: function(data, textStatus) {
					...
					$('.alert-fail').delay(1000).fadeOut();
				},
				error: function(xhr, textStatus, errorThrown) {
					$('.alert-sucess').delay(1000).fadeOut();
					$('.alert-fail').delay(1000).fadeIn();
				}
			});
		}
	});
...
function initMap() {
	var position = {lat: 7.044774, lng: 125.527087};
